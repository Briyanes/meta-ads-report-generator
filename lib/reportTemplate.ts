/**
 * Complete HTML Report Template with React + Tailwind CSS
 * All 13 slides included
 */

export function generateReactTailwindReport(analysisData: any, reportName?: string): string {
                ...
                <div className="min-h-screen bg-gray-50">
                    ...
                    <div className="bg-white p-8 min-h-screen flex items-center border-t-4 border-hadona-blue">
                        <div className="max-w-6xl mx-auto w-full text-center">
                            ...
                            <h1 className="text-4xl font-bold text-hadona-blue mb-3">Weekly over Weekly Reporting</h1>
                            {reportName && reportName !== 'Weekly Report' && (
                                <p className="text-xl font-semibold text-hadona-blue mb-4">{reportName}</p>
                            )}
                            <p className="text-lg text-gray-500 mb-6">Week vs Week</p>
                        ...
                        <div className="max-w-6xl mx-auto">
                            ...
                            <div className="grid grid-cols-2 gap-8">
                                <div className="bg-blue-50 p-6 rounded-lg border-2 border-hadona-blue">
                                    <h3 className="text-lg font-semibold text-hadona-blue mb-3">Minggu Ini</h3>
                                ...
                                <div className="bg-gray-50 p-6 rounded-lg border-2 border-gray-300">
                                    <h3 className="text-lg font-semibold text-gray-600 mb-3">Minggu Lalu</h3>
                                    <div className="space-y-4">
                                        <div className="flex justify-between">
                                            <span>Spend</span>
                                        ...
                                        <div className="flex justify-between">
                                            <span>Result (WA Started)</span>
                                            <span className="font-bold">{formatNumber(lastWeek.messagingConversations || 0)}</span>
                                        </div>
                                        <div className="flex justify-between">
                                            <span>CPR</span>
                                ...
                                <table className="w-full border-collapse border border-gray-300 text-sm">
                                    <thead>
                                        <tr className="bg-hadona-blue text-white">
                                            ...
                                            <th className="border border-gray-300 p-2 text-right text-xs">Minggu Lalu</th>
                                            <th className="border border-gray-300 p-2 text-right text-xs">Minggu Ini</th>
                                    ...
                                    <tbody>
                                        ...
                                        <tr className="bg-gray-50">
                                            ...
                                            <td className="border border-gray-300 p-2 text-right text-xs">{formatNumber(lastWeek.messagingConversations || 0)}</td>
                                        ...
                                        <tr>
                                            ...
                                            <td className="border border-gray-300 p-2 text-right text-xs">{formatCurrency(lastWeek.costPerWA || 0)}</td>
                                ...
                                <div className="p-3 bg-blue-50 rounded-lg">
                                    <div className="flex items-start">
                                        ...
                                        <div className="flex-1">
                                            ...
                                            <p className="text-xs mt-1">{resultsGrowth >= 0 ? 'Engagement' : 'Cost efficiency'} menjadi {resultsGrowth >= 0 ? 'driver utama' : 'area perbaikan'} minggu ini.</p>
                                        </div>
                                    </div>
                                </div>
                    ...
                    <div className="bg-white p-8 border-t-4 border-hadona-blue">
                        <div className="max-w-6xl mx-auto">
                            <h2 className="text-2xl font-bold text-hadona-blue mb-4">Week-on-Week Analysis</h2>
                                ...
                                <div className="border-r-2 border-gray-300 pr-6">
                                    ...
                                    <ul className="space-y-3">
                                        <li className="flex items-start">
                                            ...
                                            <span><strong>Messaging Conversations</strong> {resultsGrowth >= 0 ? 'meningkat' : 'menurun'} {Math.abs(resultsGrowth).toFixed(1)}% dengan {resultsGrowth >= 0 ? 'peningkatan' : 'penurunan'} kualitas lead</span>
                                        </li>
                                        <li className="flex items-start">
                                            <i className="fas fa-check-circle text-green-500 mt-1 mr-2"></i>
                    ...
                    ${generateBreakdownSlides(breakdown, thisWeek, lastWeek)}
...
function generateBreakdownSlides(breakdown: any, thisWeek: any, lastWeek: any): string {
  let slides = ''
  ...
  if (ageThisWeek.length > 0 || ageLastWeek.length > 0) {
    ...
    slides += `
          ...
          const result = item['Messaging conversations started'] || 0
          return `<div className="flex justify-between items-center">
                                ...
                                <div className="pl-6">
                                    <h3 className="text-base font-semibold mb-3">CPR (Cost Per Result)</h3>
                                    <div className="space-y-2">
                                        ${sortedAge.map((item: any) => {
          const age = item.Age || 'Unknown'
          const cpr = item['Cost per messaging conversation started'] || 0
          return `<div className="flex justify-between items-center">
                                            <span className="text-xs">${age}</span>
  ...
  if (platformThisWeek.length > 0) {
    // Sort by messaging conversations (highest first)
    const sortedPlatform = [...platformThisWeek]
      .filter((p: any) => p.Platform && p.Platform.trim())
      .sort((a: any, b: any) => {
        const convA = a['Messaging conversations started'] || 0
        const convB = b['Messaging conversations started'] || 0
        return convB - convA
      })
    ...
    slides += `
          ...
          const conv = item['Messaging conversations started'] || 0
          return `<div className="flex justify-between items-center">
  ...
  if (creativeThisWeek.length > 0) {
    // Get top 5 performers based on: WA Results + Instagram Visits + Followers
    const sortedCreative = [...creativeThisWeek]
      .filter((c: any) => {
        const waResults = c['Messaging conversations started'] || 0
        ...
        return waResults > 0 || instagramVisits > 0 || instagramFollows > 0
      })
      .map((item: any) => {
        const waResults = item['Messaging conversations started'] || 0
        ...
        const combinedScore = (waResults * 3) + (instagramVisits * 1) + (instagramFollows * 2)
                        ...
                        <div className="max-w-6xl mx-auto">
                            ...
                            <div className="mb-3">
                                <p className="text-xs text-gray-600">
                                    ...
                                    Top 5 Iklan berdasarkan kombinasi: WA Results, Instagram Profile Visits, dan Instagram Follows
                                </p>
                            </div>
                            <div className="space-y-4">
            ...
            return `<li className="text-xs text-gray-700 flex items-start">
                                            ...
                                            <div className="grid grid-cols-3 gap-3 mt-3">
                                                <div className="bg-white p-3 rounded border">
                                                    <div className="text-xs text-gray-600 mb-1">WA Results</div>
                                                    <div className="text-sm font-bold text-green-600">{formatNumber(${waResults})}</div>
                                                </div>
                                                <div className="bg-white p-3 rounded border">
  ...
  if (objectiveThisWeek.length > 0) {
    ...
    slides += `
          ...
          return `<div>
                                            ...
                                            <div className="space-y-2">
                                                <div className="flex justify-between">
                                                    <span>Messaging Conversation</span>
                                ...
                                <div className="border-r-2 border-gray-300 pr-6">
                                    ...
                                    <ul className="space-y-2 text-sm">
                                        <li className="flex items-start">
                                            ...
                                            <span className="text-xs"><strong>Performa Minggu Ini:</strong> ${thisWeek.amountSpent >= lastWeek.amountSpent ? 'Meningkat' : 'Menurun'} ${Math.abs(((thisWeek.amountSpent || 0) - (lastWeek.amountSpent || 0)) / (lastWeek.amountSpent || 1) * 100).toFixed(1)}% dengan spend ${thisWeek.cpr <= lastWeek.cpr ? 'lebih efisien' : 'kurang efisien'}</span>
                                        </li>
                                        <li className="flex items-start">
                                            <i className="fas fa-check-circle text-green-500 mt-1 mr-2 text-xs"></i>
